...
Reglas de construcción (todas puras, todas testeables sin DOM) — el modelo es la única fuente de verdad de "¿corresponde mostrar el puente aquí?"; el componente no vuelve a decidirlo:
- `reformulation` es `null` salvo que **la pregunta esté fallada (`!isCorrect`)** Y `question.reformulation` exista Y `extrait_audio`/`option_correcte` sean strings no vacíos Y `type` sea uno de los 3 valores válidos (`nominalisation`/`synonyme`/`restructuration`) — cualquier otra combinación (pregunta correcta, campo ausente, string vacío, tipo desconocido) se trata como "no hay puente que mostrar", nunca como un bloque a medio llenar. En la práctica `type` siempre es válido cuando `reformulation` existe (la Parte A ya lo valida antes de adjuntar el campo), pero el modelo no confía en esa garantía externa — la revalida. Nótese que esta regla exige `isCorrect === false` explícitamente, no simplemente "no marcada como correcta" — una pregunta sin responder también tiene `isCorrect === false`, así que también puede mostrar el puente (coherente con que "sin responder" ya cuenta como fallada en todo el resto de `reviewModel.js`).
...
`ExamReview.jsx` consume esto de forma trivial: dentro del bloque por pregunta ya existente (después de la lista de opciones, antes del párrafo de `feedback`, hoy alrededor de la línea 265), un bloque condicionado únicamente a `question.reformulation` (la exclusión de preguntas correctas ya viene resuelta desde el modelo, el componente no repite el chequeo de `isCorrect`):
